        ...
        res.status(500).send('Server error')
         ...
         router.put('/experience',[auth,[
             ...
             router.delete('/experience/:exp_id',auth,async (req,res)=>{
                    ...
                    try{ 
                          const index=profile.experience.map(item=>item.id).indexOf(req.params.exp_id)  //indexOf returns first occurence of value
         ...
         try{
                ...
                if(profile.likes.filter(like=>like.user.toString()===req.user.id).length>0){  //.filter() creates a new array based on given condition
